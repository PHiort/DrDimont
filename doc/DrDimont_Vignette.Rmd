...
```
Using conda the python libraries will be installed in the base environment or in the currently active environment if run via command line. If the installation does not work with DrDimont's internal function please refer to a manual installation of the libraries with conda.

Alternatively the Python dependencies can be installed manually using the `requirements_pip.txt` or the `requirements_conda.txt` file. 
Note: For conda use:
```{r, echo=TRUE, warning=FALSE, eval=FALSE}
conda install -c conda-forge --file requirements_conda.txt
```
to install the python packages in your active environment. For pip use :
```{r, echo=TRUE, warning=FALSE, eval=FALSE}
