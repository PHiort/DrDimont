...
(Figure adapted from Figure 1A from Hiort et al. (2022))


## Installation

The R package `DrDimont` can be installed via CRAN. The requirements of the package will be installed when installing the package. The complete source code can be accessed through https://gitlab.com/PHiort/DrDimont.
...
The pipeline uses a Python script for an intermediate step. For differential drug response score computation, Python (3.8 or 3.9) needs to be installed on the system. The `install_python_dependencies()` function of DrDimont can be used to install the Python dependencies. The Python packages will be installed in a virtual python or conda environment called 'r-DrDimont'. Depending on which Python package manager is installed on your system, the dependencies can be installed using pip (default):
...
```
<b> ATTENTION </b>: When using pip Python version 3.8 or 3.9 must be installed on the system (currently one of the Python dependencies ('ray') only work with Python <= 3.9). When using conda Python 3.9 will be automatically installed in the conda environment. If the installation does not work with DrDimont's internal function please refer to a manual installation of the libraries with conda.

Alternatively the Python dependencies can be installed manually using the `requirements_pip.txt` or the `requirements_conda.txt` file. <b> ATTENTION </b>: Please install the python dependencies into a virtual or conda environment with the name `r-DrDimont` otherwise the execution of the Python script will not work. 

For conda use:
```{bash, echo=TRUE, warning=FALSE, eval=FALSE}
conda install -c conda-forge --file requirements_conda.txt
```
to install the python packages in your `r-DrDimont` environment. For pip use :
```{bash, echo=TRUE, warning=FALSE, eval=FALSE}
pip install -r requirements_pip.txt
```
You can download the text files from gitlab in the `inst/` directory or find them in your R package directory folder in the `DrDimont/` folder.
...
Please be aware of the Python script used in one of the pipeline steps (see Requirements above). If you have installed python and the required packages via pip then you should set the `drdimont_settings()` parameter `conda=FALSE"`. If you have installed python and the required packages via conda then set the `drdimont_settings()` parameters `conda=TRUE`. `drdimont_settings()` will automatically check if Python can be found and show a warning if it was not found.
...
See <b>Running the individual pipeline steps</b> and call `?drdimont_settings()` for further explanations of settings parameters.
...
* Hiort, Pauline et al. “DrDimont: Explainable drug response prediction from differential analysis of multi-omics networks.” bioRxiv 2022.05.31.493964. https://doi.org/10.1101/2022.05.31.493964
